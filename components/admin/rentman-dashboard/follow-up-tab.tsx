...
import type { FollowUpAllRow, FollowUpRow, Subproject } from "@/integrations/rentman/dashboardAggregate";
import { followUpAandachtAll, followUpByMonth, followUpKpis } from "@/integrations/rentman/dashboardAggregate";
...
type ViewMode = "MAAND" | "ALLES";

/** Tabblad "Opvolging" -- exact overgenomen van het referentiedashboard
 * (v6.1): filter/classificatieregels staan toegelicht in
 * integrations/rentman/dashboardAggregate.ts. "Alle maanden"-weergave (sep
 * 2026, op verzoek) toont alleen 🔴 Direct opvolgen over alle maanden heen
 * -- doorlopend/toekomstig zijn juist de projecten die nog geen aandacht
 * nodig hebben, dus die blijven hier bewust buiten beeld. */
export function FollowUpTab({ subs }: { subs: Subproject[] }) {
  ...
  const allAandacht = followUpAandachtAll(subs);
  ...
  return (
    <div className="flex flex-col gap-3.5">
      ...
      <div className="flex flex-wrap gap-1.5">
        {(
          [
            ...
            { key: "ALLES", label: `Alle maanden -- alleen 🔴 direct opvolgen (${allAandacht.length})` },
          ] as const
        ).map((opt) => (
          <button
            key={opt.key}
      ...
      {view === "MAAND" ? (
        ...
        <div className="rounded-[10px] border p-4" style={{ background: dash.panel, borderColor: dash.border }}>
          <p className="mb-3 text-[11px]" style={{ color: dash.mutedLight }}>
            Gesorteerd op langst verlopen periode eerst -- doorlopend en toekomstig staan hier niet in, zie
            &quot;Per maand&quot; voor die twee.
          </p>
          {allAandacht.length === 0 ? (
            <p className="text-sm" style={{ color: dash.mutedLight }}>Niets om op te volgen.</p>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full min-w-[620px] border-collapse text-[12px]">
                <thead>
                  <tr>
                    {["#", "Project", "Maand", "Locatie", "Open bedrag", "Periode"].map((h, i) => (
                      <th
                        key={h}
                        className="px-2.5 py-1.5 text-[10px] font-semibold uppercase"
                ...
                <tbody>
                  {allAandacht.map((r: FollowUpAllRow) => (
                    <tr key={r.id} className="border-t" style={{ borderColor: dash.border }}>
                      <td className="px-2.5 py-1.5" style={{ color: dash.mutedLight }}>{r.number ?? "-"}</td>
                      <td className="px-2.5 py-1.5" style={{ color: dash.text }}>{r.name}</td>
                      <td className="px-2.5 py-1.5" style={{ color: dash.muted }}>{formatMonthLabel(r.month)}</td>
                      <td className="px-2.5 py-1.5" style={{ color: dash.muted }}>{r.city ? `📍 ${r.city}` : "-"}</td>
                      <td className="px-2.5 py-1.5 text-right font-semibold" style={{ color: dash.red }}>{formatEuro(r.open)}</td>
                      <td className="px-2.5 py-1.5 font-semibold" style={{ color: r.expired ? dash.red : dash.muted }}>
                        {formatDate(r.period)}
      ...
      <Callout tone="info">
        <b>Filter:</b> projecten met openstaand bedrag (omzet &gt; 0, omzet − gefactureerd &gt; 1,
        gefactureerd ≥ −0,01), ongeacht status. 🔵 <b>Doorlopend</b> = naam bevat &quot;wekelijkse&quot;. 🔴 <b>Direct opvolgen</b> = periode
        verlopen. 🟡 <b>Toekomstig</b> = periode nog niet verlopen of onbekend. &quot;Per maand&quot; is gesorteerd op hoogste
        openstaand bedrag, &quot;Alle maanden&quot; op langst verlopen periode.
      </Callout>
    </div>
  );
}
